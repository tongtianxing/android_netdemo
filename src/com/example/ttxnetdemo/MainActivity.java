...
import net.babelstar.cmscruise.service.TtxNetwork;
...
//		Intent intentDeamon = new Intent("net.babelstar.cmscruise.dogservice");
//		intentDeamon.setPackage("net.babelstar.cmscruise");
//		startService(intentDeamon);
		//开始ttx网络服务，此服务主要与ttx cmsv6 server进行网络通信，进行音视频传输，位置上报等业务
		Intent intent = new Intent("net.babelstar.cmscruise.remoteservice");
		//执法仪版本用的包名
		intent.setPackage("net.babelstar.cmscruise");
		startService(intent);
		//intentDeamon.setPackage("net.babelstar.gdispatch"); 
		...
		boolean isSuccess = bindService(intent, mServerConnection, BIND_AUTO_CREATE);
	...
	private ServiceConnection mServerConnection = new ServiceConnection() {	
		...
		public void onServiceConnected(ComponentName name, IBinder bind) {
			...
			try {
				//注意，第一次绑定成功后，配置参数，会造成 net.babelstar.gdispatch.remoteservice服务挂掉，请重新运行 CMSCruise APP，再启用ttxnetdemo即可恢复正常
				//设置通道数目，1表示1通道
				mNetBind.setChnCount(1);
				//设置app是否要自动操作摄像头
				mNetBind.setUsedCamera(false);
				mNetBind.SetAccStatus(true);
				//是否开启app定位 默认true开启，false关闭
				mNetBind.IsAppPosition(true);
				...
				mNetBind.setServerAndAccount("39.108.194.249", "30011");
				//执法仪 
...
//				mNetBind.SetEchoParam(true, 109);
	...
	protected CameraInterface.CameraListener mCameraListener = new CameraInterface.CameraListener()   {
		@Override
